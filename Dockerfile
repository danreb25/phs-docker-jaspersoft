...
RUN \
    curl -SL http://sourceforge.net/projects/jasperserver/files/JasperServer/JasperReports%20Server%20Community%20Edition%206.3.0/jasperreports-server-cp-6.1.0-bin.zip -o /tmp/jasperserver.zip && \
    unzip /tmp/jasperserver.zip -d /usr/src/ && \
    mv /usr/src/jasperreports-server-cp-6.3.0-bin /usr/src/jasperreports-server && \
    rm -rf /tmp/*
